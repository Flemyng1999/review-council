    ...
    *,
    ...
    for severity in ("blocking", "major", "moderate"):
        if severity not in keep:
            continue
        for issue in issues:
            sev = issue.get("confirmed_severity") or issue.get("proposed_severity") or ""
            if sev != severity:
                continue
    ...
    for index, issue_id in enumerate(_sort_kept(kept, issue_by_id), start=1):
        ...
        comments.append(
            {
                "id": f"CMT-{index:04d}",
                "severity": issue.get("confirmed_severity") or issue.get("proposed_severity", ""),
                "anchor_id": "",
...
def _sort_kept(kept: list[str], issue_by_id: dict[str, dict]) -> list[str]:
    def sort_key(issue_id: str) -> tuple[int, int]:
        ...
        sev = SEVERITY_ORDER.get(
            issue.get("confirmed_severity") or issue.get("proposed_severity", ""),
            99,
        )
        anchor = issue.get("anchor") or {}
